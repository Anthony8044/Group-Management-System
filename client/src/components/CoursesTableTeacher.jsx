...
export default function CoursesTableTeacher() {
    const theme = useTheme();
    const userId = React.useContext(UserContext);

    const { data: TtableData } = useGetTeacherTableQuery(userId?.user_id, {
        skip: userId?.role === "Teacher" ? false : true
    });

    return (
        <Card elevation={5} style={{ marginTop: '30px', height: '520px' }}>
            <CardContent style={{ margin: theme.spacing(2) }}>
                ...
                {TtableData ?
                    ...
                    :
                    <Typography variant="h6" textAlign={'center'}>You have not created any courses. Please create courses to view the course overview table.</Typography>
                }
            </CardContent>
        </Card>
    );
